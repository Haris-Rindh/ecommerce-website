...
import { ChevronRight, ChevronUp, ChevronDown, LayoutGrid, List, Heart, Filter, ArrowLeft } from 'lucide-react';
...
const DualRangeSlider = ({ min, max, setPriceRange }) => {
    ...
    return (
        <div className="relative w-full h-6 flex items-center mb-3 mt-1">
            ...
            <input type="range" min="0" max={MAX_SLIDER_VALUE} value={min || 0} onChange={e => setPriceRange(prev => ({...prev, min: Math.min(Number(e.target.value), Number(prev.max || MAX_SLIDER_VALUE) - 10)}))} className="absolute w-full appearance-none bg-transparent pointer-events-none [&::-webkit-slider-thumb]:pointer-events-auto [&::-webkit-slider-thumb]:w-5 [&::-webkit-slider-thumb]:h-5 [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:bg-white [&::-webkit-slider-thumb]:border [&::-webkit-slider-thumb]:border-gray-300 [&::-webkit-slider-thumb]:shadow-md [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:cursor-grab z-20" />
            <input type="range" min="0" max={MAX_SLIDER_VALUE} value={max || MAX_SLIDER_VALUE} onChange={e => setPriceRange(prev => ({...prev, max: Math.max(Number(e.target.value), Number(prev.min || 0) + 10)}))} className="absolute w-full appearance-none bg-transparent pointer-events-none [&::-webkit-slider-thumb]:pointer-events-auto [&::-webkit-slider-thumb]:w-5 [&::-webkit-slider-thumb]:h-5 [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:bg-white [&::-webkit-slider-thumb]:border [&::-webkit-slider-thumb]:border-gray-300 [&::-webkit-slider-thumb]:shadow-md [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:cursor-grab z-20" />
        </div>
    );
};
...
const SearchPage = () => {
    ...
    const [openSections, setOpenSections] = useState({ brands: true, features: true, price: true, condition: true, ratings: true });
        ...
        const fetchAndSearchProducts = async () => {
            ...
            try {
                ...
                const matchedProducts = data.filter(product => 
                    ...
                    product.categorySlug?.toLowerCase().includes(queryParam) ||
                    product.brand?.toLowerCase().includes(queryParam)
                );
                
                setProducts(matchedProducts);
                setAvailableBrands([...new Set(matchedProducts.map(p => p.brand).filter(Boolean))]);
                
                const featuresSet = new Set();
                matchedProducts.forEach(p => { if (Array.isArray(p.features)) p.features.forEach(f => featuresSet.add(f)); });
    ...
    const handleSeeAll = (type) => {
        if (type === 'brands') {
            ...
            if (isExpanding) setOpenSections({ brands: true, features: false, price: false, condition: false, ratings: false });
            else setOpenSections({ brands: true, features: true, price: true, condition: true, ratings: true });
        } else if (type === 'features') {
            const isExpanding = !showAllFeatures;
            setShowAllFeatures(isExpanding);
            if (isExpanding) setOpenSections({ brands: false, features: true, price: false, condition: false, ratings: false });
            else setOpenSections({ brands: true, features: true, price: true, condition: true, ratings: true });
        }
    };
    ...
    return (
        <div className='mx-auto max-w-[1180px] py-2 md:py-6 font-sans px-0 md:px-4'>
            <div className='md:hidden flex items-center justify-between px-4 py-3 bg-white border-b border-gray-200 mb-2'>
                <div className="flex items-center gap-2">
                    ...
                    <span className="font-semibold text-[17px] text-gray-900 capitalize">{queryParam || "Search"}</span>
                </div>
            </div>

            <div className='hidden md:flex items-center gap-2 text-gray-500 text-sm mb-6 px-4 md:px-0'>
                <Link to="/" className='hover:text-blue-600 transition-colors'>Home</Link>
                <ChevronRight size={16} />
                <span className='text-gray-900 font-medium'>Search: "{queryParam}"</span>
            </div>

            <div className='flex flex-col md:flex-row gap-0 md:gap-6 px-4 md:px-0'>
                {isMobileSidebarOpen && (
                     <div className="md:hidden">
                        <div className="fixed inset-0 bg-black bg-opacity-50 z-40" onClick={() => setIsMobileSidebarOpen(false)} />
                        <aside className='fixed top-0 right-0 bottom-0 w-[280px] bg-white z-50 overflow-y-auto p-4 animate-slide-in-right'>
                            <div className="flex items-center justify-between mb-4 border-b pb-2">
                                <h3 className="font-bold text-lg">Filters</h3>
                                <button onClick={() => setIsMobileSidebarOpen(false)} className="text-gray-500 text-2xl leading-none">&times;</button>
                            </div>
                        </aside>
                     </div>
                )}
                
                <aside className={`w-full md:w-[240px] shrink-0 flex flex-col gap-1 ${isMobileSidebarOpen ? 'fixed top-[60px] left-4 bottom-4 w-60 z-50 bg-white shadow-xl rounded-lg p-4 overflow-y-auto block' : 'hidden md:flex'}`}>
                    {isMobileSidebarOpen && (
                        ...
                        <div className='flex flex-col border-t border-gray-200 pt-4 pb-2'>
                            ...
                            {openSections.brands && (
                                <div className='flex flex-col gap-2.5'>
                                    ...
                                    {availableBrands.length > 5 && <button onClick={() => handleSeeAll('brands')} className='text-left pt-1 text-[14px] font-medium text-blue-600 hover:underline'>{showAllBrands ? "See less" : "See all"}</button>}
                                </div>
                            )}
                        </div>
                    )}
                        ...
                        <div className='flex flex-col border-t border-gray-200 pt-4 pb-2'>
                            ...
                            {openSections.features && (
                                <div className='flex flex-col gap-2.5'>
                                    ...
                                    {availableFeatures.length > 5 && <button onClick={() => handleSeeAll('features')} className='text-left pt-1 text-[14px] font-medium text-blue-600 hover:underline'>{showAllFeatures ? "See less" : "See all"}</button>}
                                </div>
                            )}
                        </div>
                    )}
                        ...
                        {openSections.price && (
                            <div className="flex flex-col gap-3">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <div className="flex-1"><label className="text-xs text-gray-500 mb-1 block">Max</label><input type="number" placeholder="2000" value={priceRange.max} onChange={e => setPriceRange({...priceRange, max: e.target.value})} className="w-full p-2 border border-gray-300 rounded text-sm focus:outline-none focus:border-blue-500 bg-white" /></div>
                                </div>
                ...
                <main className="flex-1 w-full overflow-hidden">
                    <div className="p-3 mb-4 bg-white border border-gray-200 md:rounded-md flex flex-col md:flex-row md:items-center justify-between shadow-sm gap-3 mx-[-16px] md:mx-0 px-4 md:px-3">
                        <span className="hidden md:inline text-gray-900 text-[15px]">{filteredProducts.length} items found for <strong>"{queryParam}"</strong></span>
                        <div className='flex justify-between md:justify-end gap-2 md:gap-4 items-center w-full md:w-auto'>
                            ...
                            <div className="flex gap-2 items-center md:hidden flex-1">
                                <button onClick={() => setIsFilterBoxOpen(!isFilterBoxOpen)} className="flex-1 px-3 py-1.5 flex items-center justify-center gap-1 border border-gray-300 rounded text-gray-700 bg-white text-[13px] font-medium">
                                    Sort: {selectedFilter.name.split(':')[0]} <ChevronDown size={14} />
                                </button>
                                <button onClick={() => setIsMobileSidebarOpen(true)} className="flex-1 px-3 py-1.5 flex items-center justify-center gap-1 border border-gray-300 rounded text-gray-700 bg-white text-[13px] font-medium">
                    ...
                    {(selectedBrands.length > 0 || selectedFeatures.length > 0 || selectedRatings.length > 0 || condition !== 'Any' || appliedPrice.min !== '') && (
                        <div className="flex items-center gap-2 mb-4 overflow-x-auto no-scrollbar pb-1">
                            {selectedBrands.map((brand) => <span key={brand} className="shrink-0 px-3 py-1 bg-white border border-blue-200 rounded-full text-blue-600 text-[13px] flex items-center gap-2 shadow-sm">{brand} <button onClick={() => toggleArrayFilter(selectedBrands, setSelectedBrands, brand)} className="hover:text-red-500 font-bold">×</button></span>)}
                            {selectedFeatures.map((feature) => <span key={feature} className="shrink-0 px-3 py-1 bg-white border border-blue-200 rounded-full text-blue-600 text-[13px] flex items-center gap-2 shadow-sm">{feature} <button onClick={() => toggleArrayFilter(selectedFeatures, setSelectedFeatures, feature)} className="hover:text-red-500 font-bold">×</button></span>)}
                            <button onClick={clearAllFilters} className="shrink-0 text-gray-500 text-[13px] font-medium hover:text-red-500 hover:underline ml-1">Clear all filters</button>
                        </div>
                    )}

                    {isLoading ? (
                        <div className="w-full py-20 flex items-center justify-center text-lg text-gray-500 font-medium">Searching inventory...</div>
